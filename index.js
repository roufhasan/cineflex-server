...
async function run() {
  try {
    ...
    app.delete("/watchlist/id/:id", async (req, res) => {
    ...
    // Delete a user's full watchlist
    app.delete("/watchlist/email/:email", async (req, res) => {
      const email = req.params.email;
      const query = { email: email };

      const result = await watchListCollection.deleteMany(query);
      res.send(result);
    });
